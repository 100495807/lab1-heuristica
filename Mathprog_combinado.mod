...
param number_seats{AIRPLANE}, >= 0;                                     # Número de asientos disponibles en cada avión
param capacity{AIRPLANE}, >= 0;                                         # Capacidad de equipaje en cada avión (en kg)

# Parámetros para las tarifas
param price{FARES}, >= 0;                                               # Precio de cada tarifa
param weight{FARES}, >= 0;                                              # Peso del equipaje asociado a cada tarifa (en kg)
...
var billetes{AIRPLANE, FARES} >= 0, integer;                                   # Número de billetes vendidos por avión y tarifa
...
# Función objetivo
maximize Net_Profit:
    sum{i in AIRPLANE, j in FARES} price[j] * billetes[i,j] - sum {a in AIRPLANE, r in RUNWAY, s in SLOT} delay_cost[a] * (possible_arrival_time[s] - scheduled_arrival[a]) * availability[a,r,s];

# Restricciones:
# 1. No superar el número de asientos disponibles en cada avión
s.t. Seat_Constraint{i in AIRPLANE}:
    sum{j in FARES} billetes[i,j] <= number_seats[i];

# 2. No superar la capacidad de equipaje en cada avión
s.t. Capacity_Constraint{i in AIRPLANE}:
    sum{j in FARES} weight[j] * billetes[i,j] <= capacity[i];

# 3. Deben venderse al menos 20 billetes LeisurePlus y 10 billetes BusinessPlus en cada avión
s.t. Min_LeisurePlus{i in AIRPLANE}:
    billetes[i, "LeisurePlus"] >= minimo_leisure;

s.t. Min_BusinessPlus{i in AIRPLANE}:
...
s.t. Standard_Proportion:
    sum{i in AIRPLANE} billetes[i, "Standard"] >= 0.6 * sum{i in AIRPLANE, j in FARES} billetes[i,j];
